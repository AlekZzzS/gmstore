'use strict';
// Данные из «Приложение. Материалы» тестового задания.
const { tx, run, get, all, nowIso } = require('./db');
...
/** Идемпотентный посев: повторный запуск ничего не ломает и не дублирует. */
function seed() {
  tx(() => {
    for (const p of PRODUCTS) {
      run(
        ...
        p.sku, p.name, p.type, p.price * 100, p.currency, p.image
      );
    }
    ...
    for (const pc of PROMOCODES) {
      run(
        ...
        pc.code, pc.type, pc.value, pc.currency, pc.max_uses
      );
    }

    // Пул из 50 ключей делится между двумя поставщиками-заглушками.
    const existing = get('SELECT COUNT(*) c FROM provider_stock').c;
    if (existing === 0) {
      const nA = Math.max(0, Math.min(config.stockSplit.A, KEYS.length));
      KEYS.forEach((code, i) => {
        run(
          'INSERT INTO provider_stock(provider, code) VALUES (?,?)',
          i < nA ? 'A' : 'B', code
        );
      });
    }
...
module.exports = { seed, PRODUCTS, KEYS, PROMOCODES };
